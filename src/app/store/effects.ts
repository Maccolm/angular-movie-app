...
import { loadFavoriteMovies, loadFavoriteMoviesFailure, loadFavoriteMoviesSuccess, loadMovies, loadMoviesFailure, loadMoviesSuccess, loadWatchList, loadWatchListFailure, loadWatchListSuccess } from './actions';
		...
		this.actions$.pipe(
			...
			mergeMap(() => {
				return this.movieService.getFavoriteMovies().pipe(
					map(favoriteMovies => loadFavoriteMoviesSuccess({ favoriteMovies })),
